...
function CommandItem({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "relative flex cursor-pointer items-center gap-2 rounded-[var(--radius)] px-2 py-2 text-sm outline-none select-none data-[selected=true]:bg-muted data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50",
        className,
      )}
      {...props}
    />
